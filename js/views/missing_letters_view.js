...
export default class MissingLettersView {
  ...
  render() {
    ...
    for (let i = 0; i < parts.length; i++) {
      ...
      if (i < this.model.blanks.length)
        html += `<input type="text" maxlength="1" class="form-control mx-2 text-center fw-bold fs-4 rounded-3 shadow-sm lexis-input-blank" data-index="${i}">`;
    }
    c.innerHTML = `
    ...
    c.querySelector("#missing-submit").addEventListener(
      "click",
      this._onSubmit,
    );
  ...
  _onSubmit() {
    ...
    if (this.model.checkAnswers(inputs.map((i) => i.value.trim()))) {
      playCorrect();
      inputs.forEach((inp) =>
        inp.classList.add("lexis-correct-pulse", "lexis-flash-correct"),
      );
      setTimeout(() => {
        c.dispatchEvent(
